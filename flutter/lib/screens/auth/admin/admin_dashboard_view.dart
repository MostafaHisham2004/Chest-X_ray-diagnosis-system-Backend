...
class _AdminDashboardViewState extends State<AdminDashboardView>
  ...
  Future<void> _approveDoctor(ManagedUser doc) async {
    ...
    try {
      ...
      setState(() => _processingIds.remove(doc.id));
      _showSnack('Could not approve doctor.');
    }
  }
  ...
  Future<void> _rejectDoctor(ManagedUser doc) async {
    ...
    try {
      ...
      setState(() => _processingIds.remove(doc.id));
      _showSnack('Could not reject doctor.');
    }
  }
  ...
  Future<void> _suspendUser(ManagedUser user) async {
    ...
    try {
      ...
      setState(() => _processingIds.remove(user.id));
      _showSnack('Could not suspend user.');
    }
  }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(icon, size: 48, color: AppTheme.primary.withOpacity(0.4)),
            const SizedBox(height: 16),
            Text(
              title,
              style: GoogleFonts.dmSans(
